...
Run liquibase diff to generate changelogs

```shell
./mvnw liquibase:diff
```

Add the new changlog in `src/main/resources/config/liquibase/changelog/` to `master.xml` in the liquibase folder apply the changelog

```xml
<include file="config/liquibase/changelog/20241222151344_changelog.xml" relativeToChangelogFile="false"/>
```

Then run `./mvnw` to apply changes to db
...
```shell
./mvnw package -Pprod,no-liquibase verify jib:dockerBuild -DskipTests
```
...
```shell
docker compose -p warehouse -f src/main/docker/app.yml up -d
```

for uat (anfo.asia)

```shell
docker compose -p warehouse -f src/main/docker/uat/app-uat.yml up -d
```

for prod (supertrungviet)

```shell
docker compose -p warehouse -f src/main/docker/prod/app-prod.yml up -d
```
